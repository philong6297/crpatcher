...
from tests.unittests.test_config_2.arg_builder import ARG_BUILDER
...
def fixt_patch_request(
    ...
    repo_dir = ARG_BUILDER.build_repo_dir(
    ...
    patch_dir = ARG_BUILDER.build_patch_dir(
    ...
    ignore_patterns = ARG_BUILDER.build_ignore_patterns(ignore_patterns_arg)

    keep_patch_files = ARG_BUILDER.build_keep_patch_files(keep_patch_files_arg)

    program_context = ARG_BUILDER.build_program_context(
        has_context=program_context_arg,
        file=fixt_crpatcher_existing_empty_file,
    )
